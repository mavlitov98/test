<?php

use Illuminate\Http\Request;
use Illuminate\Support\Facades\Route;

/*
|--------------------------------------------------------------------------
| API Routes
|--------------------------------------------------------------------------
|
| Here is where you can register API routes for your application. These
| routes are loaded by the RouteServiceProvider within a group which
| is assigned the "api" middleware group. Enjoy building your API!
|
*/

Route::middleware('auth:api')->get('/user', function (Request $request) {
    return $request->user();
});

//запрос для записи информации о пользователе
Route::post('/neworder', 'ApiController@createClient');

//запрос для получения записи по id заказах
Route::get('/order/{id}', 'ApiController@getOrderByID');

//запрос для получения всех записей о заказах
Route::get('/orders', 'ApiController@getOrders');

//запрос для получения записей о тарифах
Route::get('/rates', 'RateController@getRates');

//запрос для получения записи о тарифе по id
Route::get('/rates/{id}', 'RateController@getRatesByID');
